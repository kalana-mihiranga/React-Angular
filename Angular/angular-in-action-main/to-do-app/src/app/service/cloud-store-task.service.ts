...
export class CloudStoreTaskService implements TaskService {
  ...
  constructor(private db: Firestore,
    ...
    const taskListQuery = query(collection(db, "tasks"), where("email", "==", authService.getPrincipal()?.email));
  ...
  createTask(description: string): Promise<void> {
    const taskDTO = new TaskDTO(null, description, false, this.authService.getPrincipal()!.email!);
